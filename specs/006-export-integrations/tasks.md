...
- [ ] T002 Inspect `ExportPanel`, `ExportsPage`, `Dashboard`, `WorkflowWorkspace`,
  the session store, export API routes, and current `mockExportPackages` usage.
...
- [ ] T009 Add API endpoint or service to produce Markdown/CSV/JSON from
      persisted export records.
- [ ] T010 Validate package creation inputs at the server boundary.

## Local Export UI

- [ ] T011 Wire ExportPanel to create persisted packages.
...
- [ ] T019 Add Jira export endpoint or service if configured.
...
- [ ] T023 Add GitHub issue export endpoint or service if configured.
- [ ] T024 Record GitHub per-item success/failure and remote URLs.
- [ ] T025 Add retry path for failed items where safe.

## Security And Report

- [ ] T026 Ensure no tokens or secrets are hardcoded.
